...
void scan(String domain, int start, int end,
    {int delay = 0, int timeout = 200}) async {
  ...
  final progress = Progress(
	...
	for (int port = start; port <= end; port++) {
	  ...
	  try{
			...
			final socket = await Socket.connect(domain, port, timeout : Duration(milliseconds: timeout));
			socket.destroy();
...
void main(List<String> arguments) {
  ...
  parser
    ...
    ..addOption("delay",
        abbr: "d",
        help: "number of seconds to wait between port scanning",
        defaultsTo: "0",
        callback: (String? val) => {
              if (val != null)
                {
                  if (int.parse(val) < 0)
                    {throw IndexError(int.parse(val), 600)}
                      ...
                      600 /*must be less than 10 minutes or 600 seconds*/)
                    {throw IndexError(int.parse(val), 600)}
                }
            })
    ..addOption("timeout",
        abbr: "t",
        help: "number of miliseconds to wait for an irresponsive port",
        defaultsTo: "200",
        callback: (String? val) => {
              if (val != null)
                {
                  if (int.parse(val) < 200)
                    {throw IndexError(int.parse(val), 20000)}
                  else if (int.parse(val) >
                      20000 /*must be less than 20 seconds*/)
                    {throw IndexError(int.parse(val), 20000)}
                }
            })
  ...
  scan(argsResult["domain"], int.parse(argsResult["start"]),
